...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-box { padding: 1.2rem; text-align: center; border-bottom: 1px solid var(--border-soft); }
        .stat-box:last-child { border-bottom: none; }
        .stat-label { font-size: 0.75rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase; margin-bottom: 0.3rem; letter-spacing: 0.5px; }
        .stat-value { font-size: 1.8rem; color: var(--brand-1); font-weight: 700; }
        .stat-chart { width: 100%; height: 200px; margin-top: 1rem; }

        .form-group { margin-bottom: 1rem; }
        .form-label { display: block; font-size: 0.85rem; font-weight: 600; color: var(--text-muted); margin-bottom: 0.4rem; letter-spacing: 0.5px; }
        .form-control { width: 100%; padding: 0.6rem 0.8rem; border: 2px solid var(--border-soft); border-radius: 6px; font-size: 0.85rem; font-family: inherit; color: var(--text-primary); outline: none; background: var(--input-bg); transition: all 0.3s ease; }
        .form-control:focus { border-color: var(--brand-1); background: #fff; box-shadow: 0 0 0 3px rgba(47, 122, 89, 0.12); }
        textarea.form-control { resize: vertical; min-height: 80px; }
        
        .btn-submit { padding: 0.6rem 1.5rem; background: linear-gradient(135deg, var(--brand-1) 0%, var(--brand-2) 100%); color: #fff; border: none; border-radius: 5px; font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 2px 8px rgba(47, 122, 89, 0.25); }
        .btn-submit:hover { transform: translateY(-1px); background: linear-gradient(135deg, var(--brand-1-strong) 0%, var(--brand-2-strong) 100%); }

        .alert { padding: 0.6rem 1rem; border-radius: 5px; font-size: 0.8rem; margin-bottom: 1rem; font-weight: 600; }
        ...
        .ann-item { padding: 0.8rem 0; border-bottom: 1px solid var(--border-soft); }
        .ann-item:last-child { border-bottom: none; }
        .ann-date { font-size: 0.7rem; color: var(--text-muted); font-weight: 700; margin-bottom: 0.2rem; }
        .ann-text { font-size: 0.8rem; color: var(--text-primary); line-height: 1.4; }

        table { width: 100%; border-collapse: collapse; font-size: 0.8rem; margin-top: 1rem; }
        table th { background: var(--border-soft); padding: 0.5rem; text-align: left; font-weight: 700; color: var(--text-primary); }
        table td { padding: 0.5rem; border-bottom: 1px solid var(--border-soft); }
...
<div class="admin-wrap">
    <div class="admin-grid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <?php if ($ann_error): ?><div class="alert alert-error"><?= htmlspecialchars($ann_error) ?></div><?php endif; ?>
                <?php if ($ann_success): ?><div class="alert alert-success"><?= htmlspecialchars($ann_success) ?></div><?php endif; ?>
                ...
                <div style="margin-top: 1.5rem;">
                    <h3 style="font-size: 0.85rem; margin-bottom: 0.8rem; font-family: 'Merriweather', serif; font-weight: 600; color: var(--text-primary);">Posted Announcement</h3>
