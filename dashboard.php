...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .d-nav-links li a:hover { background: rgba(255,255,255,0.14); }
        .d-logout {
        ...
        .d-flash-ok  { background: #e6f4ea; color: #155724; border: 1px solid #b7dfbe; }
        .d-flash-err { background: #fde8e8; color: #a01a1a; border: 1px solid #f5b7b7; }
        ...
        .d-info-list li b { font-weight: 700; }
        ...
        .d-rules-uni    { text-align:center; font-weight:700; font-size:0.92rem; font-style:italic; color: var(--text-primary); font-family: 'Merriweather', serif; }
        .d-rules-col    { text-align:center; font-size:0.76rem; font-weight:700; text-transform:uppercase; letter-spacing:0.03em; color: var(--text-primary); }
        .d-rules-title  { font-size:0.8rem; font-weight:700; text-transform:uppercase; letter-spacing:0.04em; color: var(--text-primary); margin-top:0.15rem; }
        .d-rules-intro  { font-size:0.81rem; color: var(--text-muted); }
        .d-rules-body p { font-size:0.81rem; color: var(--text-muted); }
        ...
        .ef-control[readonly] { background: #f4f4f4; cursor: not-allowed; color: var(--text-muted); }
        select.ef-control {
            appearance: none;
        ...
        .ef-btn-cancel:hover { background: #d0d0d0; transform: translateY(-2px); }
    </style>
</head>
<body>
...
<?php if (isset($_SESSION["success"])): ?>
    <div class="d-flash d-flash-ok"><?= htmlspecialchars($_SESSION["success"]) ?><?php unset($_SESSION["success"]); ?></div>
<?php endif; ?>
<?php if (isset($_SESSION["error"])): ?>
    <div class="d-flash d-flash-err"><?= htmlspecialchars($_SESSION["error"]) ?><?php unset($_SESSION["error"]); ?></div>
<?php endif; ?>
            ...
            <form method="POST" action="update_profile.php">
                <div class="ef-grid">
                    <div class="ef-group ef-full">
                        ...
                        <input class="ef-control" type="text" value="<?= htmlspecialchars($user["id_number"]) ?>" readonly>
                    </div>
                    <div class="ef-group ef-full">
                        <label class="ef-label">First Name</label>
                        <input class="ef-control" type="text" name="first_name" value="<?= htmlspecialchars($user["first_name"]) ?>" required>
                    </div>
                    <div class="ef-group ef-full">
                        <label class="ef-label">Last Name</label>
                        <input class="ef-control" type="text" name="last_name" value="<?= htmlspecialchars($user["last_name"]) ?>" required>
                    </div>
                    <div class="ef-group ef-full">
                        <label class="ef-label">Middle Name</label>
                        <input class="ef-control" type="text" name="middle_name" value="<?= htmlspecialchars($user["middle_name"] ?? "") ?>">
                    </div>
                    <div class="ef-group">
                        ...
                        <select class="ef-control" name="course" required>
                            ...
                            <option value="BSCA" <?= $user["course"]==="BSCA"?"selected":"" ?>>BSCA</option>
                    ...
                    <div class="ef-group">
                        ...
                        <select class="ef-control" name="course_level" required>
                            <?php for($i=1;$i<=4;$i++): ?>
                            <option value="<?=$i?>" <?=(string)$user["course_level"]===(string)$i?"selected":"" ?>><?=$i?></option>
                            <?php endfor; ?>
                        </select>
                    </div>
                    <div class="ef-group ef-full">
                        <label class="ef-label">Email</label>
                        <input class="ef-control" type="email" name="email" value="<?= htmlspecialchars($user["email"]) ?>" required>
                    </div>
                    <div class="ef-group ef-full">
                        <label class="ef-label">Address</label>
                        <input class="ef-control" type="text" name="address" value="<?= htmlspecialchars($user["address"] ?? "") ?>">
                    </div>
                </div>
    ...
    <div class="d-card">
        ...
        <div class="d-card-body">
            ...
            <ul class="d-info-list">
                <li>👤 <span><b>Name:</b> <?= htmlspecialchars($user["first_name"] . " " . $user["last_name"]) ?></span></li>
    ...
    <div class="d-card">
        ...
        <div class="d-card-body">
            <div class="d-rules-body">
                ...
                <p class="d-rules-intro">To avoid embarrassment and maintain camaraderie with your friends and superiors at our laboratories, please observe the following:</p>
